...
private final class OrbView: NSView {
    ...
    private func drawLabels(in rect: NSRect) {
        ...
        let subtitle = active.map { "\(windowName($0.windowMinutes))剩余" } ?? "暂无数据"
    ...
    private func updateToolTip() {
        guard let snapshot = snapshot else {
            toolTip = "尚未找到 Codex 用量数据，请先完成一次 Codex 对话。"
            return
        }
        var lines = ["Codex 剩余用量"]
        ...
        for item in snapshot.windows.sorted(by: { $0.windowMinutes < $1.windowMinutes }) {
            ...
            lines.append("\(windowName(item.windowMinutes))：\(Int(item.remainingPercent.rounded()))%（\(reset) 重置）")
        }
        if let plan = snapshot.planType { lines.append("方案：\(plan)") }
...
private final class AppearanceControls: NSObject {
    ...
    var onChange: ((CGFloat, NSColor) -> Void)?
    ...
    init(size: CGFloat, color: NSColor) {
        ...
        sizeRow.spacing = 8
        let sizeTitle = NSTextField(labelWithString: "尺寸")
        ...
        colorRow.spacing = 6
        for preset in [("绿色", "#61DC18"), ("蓝色", "#37BEFF"), ("紫色", "#A970FF"), ("橙色", "#FF9F35")] {
    ...
    @objc private func valueChanged() {
        ...
        onChange?(CGFloat(slider.doubleValue), colorWell.color)
    }
...
private final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, OrbViewDelegate {
    ...
    func orbView(_ view: OrbView, showMenuFor event: NSEvent) {
        let menu = NSMenu(title: "Codex 用量悬浮球")
        menu.addItem(withTitle: "立即刷新", action: #selector(refreshFromMenu), keyEquivalent: "")
        menu.addItem(withTitle: "外观设置…", action: #selector(showAppearance), keyEquivalent: "")
    ...
    @objc private func showAppearance() {
        ...
        let controls = AppearanceControls(size: size, color: accent)
        controls.onChange = { [weak self] newSize, newColor in self?.apply(size: newSize, accent: newColor) }

        let alert = NSAlert()
        alert.messageText = "实时调整大小和颜色"
        alert.informativeText = "尺寸范围为 50–300 px，修改会立即预览。"
        ...
        if response == .alertFirstButtonReturn {
            ...
            apply(size: originalSize, accent: originalAccent)
        }
    }

    private func apply(size newSize: CGFloat, accent newAccent: NSColor) {
...
private func windowName(_ minutes: Int) -> String {
    if (10_000...10_200).contains(minutes) { return "周限额" }
    if (280...320).contains(minutes) { return "5小时限额" }
    if minutes % 1_440 == 0 { return "\(minutes / 1_440)天限额" }
